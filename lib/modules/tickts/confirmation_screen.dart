...
class _ConfirmationScreenState extends State<ConfirmationScreen> {
  ...
  Future<void> _loadUserBenefits() async {
    ...
    // الخصم بييجي بس من شاشة الحالة — مش من الـ API
  ...
  Widget build(BuildContext context) {
    ...
    // الخصم يطبق تلقائي لو في حالة مختارة
    double discountedPrice = _hasDiscount
        ? basePrice * (1 - _discountPercent / 100)
        : basePrice;
    // السويتش بس للاشتراك
    double totalPrice = (isSwitched && _hasSubscription) ? 0 : discountedPrice;
                      ...
                      Row(children: [
                        Text(
                            ...
                            style: GoogleFonts.cairo(fontSize: 18, fontWeight: FontWeight.w700,
                                color: (_hasSubscription || _hasDiscount) ? Colors.black : Colors.grey)),
                        const SizedBox(width: 6),
                        Icon(Icons.workspace_premium,
                            color: (_hasSubscription || _hasDiscount) ? const Color(0xffF2EE08) : Colors.grey),
                      ]),
                    ],
                  ),
                  const SizedBox(height: 16),
