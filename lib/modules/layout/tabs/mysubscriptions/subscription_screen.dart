...
class _SubscriptionScreenState extends State<SubscriptionScreen> {
  ...
  Future<void> _cancelSubscription() async {
    ...
    if (result['success'] == true) {
      ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text("تم إلغاء الاشتراك", style: GoogleFonts.cairo()), backgroundColor: Colors.green));
      _loadSubscription();
    } else {
      ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text(result['message'] ?? 'فشل الإلغاء', style: GoogleFonts.cairo()), backgroundColor: Colors.red));
    }
  }
  ...
  String _planName(String? type) {
    switch ((type ?? '').toLowerCase()) {
      case 'yearly':
      case 'year':   return 'اشتراك سنوي';
      case 'monthly':
      case 'month':  return 'اشتراك شهري';
      case 'weekly':
      case 'week':   return 'اشتراك أسبوعي';
              ...
              Container(
                ...
                child: Stack(children: [
                  Positioned(left: 16, bottom: 30,
                      child: IconButton(
                          onPressed: () => Navigator.pushNamedAndRemoveUntil(context, Home.route, (r) => false, arguments: 2),
                          icon: const Icon(Icons.arrow_back), iconSize: 24, color: Colors.black)),
                  Positioned(left: 0, right: 0, bottom: 40,
                      child: Center(child: Text('اشتراكي',
              ...
              Expanded(
                child: _isLoading
                  ...
                  child: SingleChildScrollView(
                    ...
                    child: _sub == null ? _buildNoSubscription() : _buildSubscriptionCard(),
                  ),
                ),
              ),
            ],
  ...
  Widget _buildNoSubscription() {
    return Column(
      children: [
        ...
        Text("لا يوجد اشتراك نشط", style: GoogleFonts.cairo(fontSize: 20, color: Colors.grey)),
        ...
        SizedBox(
          ...
          child: ElevatedButton(
              ...
              child: Text("تصفح الباقات", style: GoogleFonts.cairo(fontSize: 20, fontWeight: FontWeight.w700, color: Colors.white))),
        ),
      ],
    );
  }
  ...
  Widget _buildSubscriptionCard() {
    final planType   = _sub!['typeSubscription'] ?? _sub!['planType'] ?? _sub!['type'] ?? '';
    ...
    final progress   = totalTrips > 0 ? (usedTrips / totalTrips).clamp(0.0, 1.0) : 0.0;
      ...
      children: [
        ...
        Container(
          ...
          decoration: BoxDecoration(
              ...
              boxShadow: [BoxShadow(color: Colors.grey.withOpacity(0.5), blurRadius: 4, offset: const Offset(0, 4))]),
          child: Padding(
            padding: const EdgeInsets.all(16),
            child: Column(children: [
              Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
                Container(
                    ...
                    decoration: BoxDecoration(
                        color: isActive ? Colors.lightGreenAccent : Colors.grey.shade300,
              ...
              Row(mainAxisAlignment: MainAxisAlignment.end, children: [
                Text("صالح : $startDate  _  $endDate",
                    style: GoogleFonts.cairo(fontWeight: FontWeight.w400, fontSize: 18, color: Colors.grey)),
              ]),
              const SizedBox(height: 20),
              Row(mainAxisAlignment: MainAxisAlignment.end, children: [
                Text("%الخصم : $discount",
              ...
              const SizedBox(height: 30),
              Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
                Text("$totalTrips / $usedTrips",
                    style: GoogleFonts.cairo(fontWeight: FontWeight.w700, fontSize: 20, color: Colors.grey)),
                Text("الرحلات المستخدمة",
                    style: GoogleFonts.cairo(fontWeight: FontWeight.w700, fontSize: 20, color: Colors.black)),
              ]),
              const SizedBox(height: 20),
              Directionality(
        ...
        // ── زر تجديد ──────────────────────────────────────────────────────
        SizedBox(
          width: double.infinity, height: 69,
          child: ElevatedButton(
        ...
        // ── سجل الاشتراكات ────────────────────────────────────────────────
        GestureDetector(
          onTap: () => Navigator.push(context, MaterialPageRoute(builder: (_) => SubscriptionRecords())),
          child: Row(mainAxisAlignment: MainAxisAlignment.center, children: [
            Text("عرض سجل الاشتراكات",
    ...
    showGeneralDialog(
      ...
      pageBuilder: (ctx, _, __) => Center(
        child: Material(
          ...
          child: Container(
            width: 355, padding: const EdgeInsets.all(20),
            decoration: BoxDecoration(color: Colors.white, borderRadius: BorderRadius.circular(16)),
            child: Column(
              mainAxisSize: MainAxisSize.min,
              children: [
                const SizedBox(height: 25),
                Text("سيتم تجديد اشتراكك الحالي بنفس الباقة",
                    textAlign: TextAlign.center,
                    style: GoogleFonts.cairo(fontSize: 16, fontWeight: FontWeight.w700)),
                const SizedBox(height: 30),
                Column(crossAxisAlignment: CrossAxisAlignment.stretch, children: [
                  Text("الاشتراك  : ${_planName(planType)}", textAlign: TextAlign.right,
                      style: GoogleFonts.cairo(fontWeight: FontWeight.w600, fontSize: 13)),
                  const SizedBox(height: 10),
                  Text("السعر  : $price جنية", textAlign: TextAlign.right,
                      style: GoogleFonts.cairo(fontWeight: FontWeight.w600, fontSize: 13)),
                ]),
                const SizedBox(height: 30),
                Row(children: [
                  Expanded(child: ElevatedButton(
                      ...
                      style: ElevatedButton.styleFrom(
                          backgroundColor: const Color(0xFFF2F2F2), foregroundColor: Colors.black,
                          elevation: 0, padding: const EdgeInsets.symmetric(vertical: 18),
                          shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(12))),
                      child: Text("إلغاء", style: GoogleFonts.cairo(fontWeight: FontWeight.w700, fontSize: 14)))),
                  const SizedBox(width: 40),
                  Expanded(child: ElevatedButton(
                      onPressed: () async {
                        Navigator.pop(ctx);
                        final accountId = await SessionManager.getAccountId() ?? 0;
                        final result = await ApiService().createSubscription(accountId: accountId, planType: planType);
                        if (!context.mounted) return;
                        if (result['success'] == true) {
                          ScaffoldMessenger.of(context).showSnackBar(SnackBar(
                              content: Text("تم التجديد بنجاح!", style: GoogleFonts.cairo()),
                              backgroundColor: Colors.green));
                          _loadSubscription();
                        } else {
                          ScaffoldMessenger.of(context).showSnackBar(SnackBar(
                              content: Text(result['message'] ?? 'فشل التجديد', style: GoogleFonts.cairo()),
                      ...
                      style: ElevatedButton.styleFrom(
                          ...
                          elevation: 0, padding: const EdgeInsets.symmetric(vertical: 18),
                          shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(12))),
                      child: Text("تأكيد", style: GoogleFonts.cairo(fontWeight: FontWeight.w700, fontSize: 14)))),
                ]),
              ],
            ),
          ),
